...
public class GrayLoadBalancer implements ReactorServiceInstanceLoadBalancer {
    ...
    public Mono<Response<ServiceInstance>> choose(Request request) {
        ...
        // 设置Header version，openFeign ribbon的依据
        //方法二 (设置Header写入权限)
//        HttpHeaders.writableHttpHeaders(headers);
        //这里get到的Header是ReadOnlyHttpHeaders，不支持修改
//        exchange.getRequest().getHeaders().add(Header.VERSION.getValue(), serverGrayProperty.getGrayVersion());
        //方法一 (mutate方法其实是重新构造request对象)
//        exchange.getRequest().mutate().header(Header.VERSION.getValue(), serverGrayProperty.getGrayVersion()).build();
        //未开启灰度发布，只走proVersion
        headers.set(Header.VERSION.getValue(), serverGrayProperty.getProVersion());
        if (log.isDebugEnabled()) {
            log.debug("current headers : {}", headers);
        }
        // 选择实例
